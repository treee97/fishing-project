...
const SellItemDetail = ({ selectedItem, onSellItem }: any) => {
	...
	const handleSell = async (e: React.FormEvent) => {
		...
		try {
			...
			if (response.status === 201) {
				// alert("item sold");
				// <Message text="The Item has been listed to the marketplace" />;
				onSellItem("The Item has been listed to the marketplace");
			} else if (response.status !== 200) {
				// <Message text="Error at listing the message" />;
	...
	return (
		<form
			className="relative border-2 border-yellow-600 w-2/4 flex flex-col text-light-text dark:text-dark-text"
			onSubmit={handleSell}
		>
